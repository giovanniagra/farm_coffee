...
logger = logging.getLogger('farm_coffee.farm_coffee_app.views')
                ...
                if df.iloc[sim_products[s], user_index] == 0:
                    if len(product_similarity_copy) == (number_neighbors - 1):
                        ...
                        product_similarity_copy.pop(
                            s - (len(product_similarity) - len(product_similarity_copy))
                        )

                else:
                    nominator = (
                        nominator
                        + product_similarity[s] * df.iloc[sim_products[s], user_index]
                    )
...
def view_product_history(request, pk):
    ...
    return render(
        request, "order/order_history_details.html", {"items": zip(items, quantity)}
    )
...
)  # check is the logged in user is manager else redirect to home page
...
def create_employee(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            messages.success(
                request, f'Employee: {form.cleaned_data["first_name"]} created'
            )
...
def order_list(request):
    ...
    context = {"orders": orders, "orderStatus": orderStatus}
